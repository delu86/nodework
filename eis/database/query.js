const _serviceHistoryQuery = {
  FEAFiliali:"select md_filiale,md_data,md_tot_err as errori , md_tot_canc as cancellati , md_tot_regulas as regolari, md_tot_revoked as revocati from md.md_fea_filiale where md_abi=? and DATEDIFF(?,md_data)<=7 order by  2,1 ;",
  FEAfirmati:"select data , errori, cancellati , regolari,revocati from (SELECT md_data as data,md_tot_err as errori,md_tot_canc as cancellati,md_tot_regulas as regolari,md_tot_revoked as revocati FROM md.wd_fea_volumi where md_abi=?  and DATEDIFF(?,md_data) BETWEEN 0 and 31 order by 1 desc  limit 20) as a order by 1 asc;",
  FEAFilialiContabili:"select md_filiale,md_data,md_tot_err as errori , md_tot_canc as cancellati , md_tot_regulas as regolari, md_tot_revoked as revocati from md.md_fea_filiale_contabili where md_abi=? and DATEDIFF(?,md_data)<=4 order by  2,1 ;",
  FEAFilialiContratti:"select md_filiale,md_data,md_tot_err as errori , md_tot_canc as cancellati , md_tot_regulas as regolari, md_tot_revoked as revocati from md.md_fea_filiale_contratti where md_abi=? and DATEDIFF(?,md_data)<=4 order by  2,1 ;",
  FEAcontratti:"select data , errori, cancellati , regolari,revocati from (SELECT md_data as data,md_tot_err as errori,md_tot_canc as cancellati,md_tot_regulas as regolari,md_tot_revoked as revocati FROM md.wd_fea_volumi_contratti where md_abi=?  and DATEDIFF(?,md_data) BETWEEN 0 and 31 order by 1 desc  limit 20) as a order by 1 asc;",
  FEAcontabili:"select data , errori, cancellati , regolari,revocati from (SELECT md_data as data,md_tot_err as errori,md_tot_canc as cancellati,md_tot_regulas as regolari,md_tot_revoked as revocati FROM md.wd_fea_volumi_contabili where md_abi=?  and DATEDIFF(?,md_data) BETWEEN 0 and 31 order by 1 desc  limit 20) as a order by 1 asc;",
...
function _getServiceDailyDetail(instituteId,serviceName,date){
    return [
       {$match:{"abi":instituteId, "data":date}}
      ,{$unwind:"$servizio"}
      ,{$match:{"servizio.nomeservizio":serviceName}}
      ,{$project:{"data":"$servizio.rilevazioni"}}
  ]
} 
...
function  _getWalldisplayChartsQuery(serviceName, instituteId , date){
  ...
  var query = {
      ...
      "FEAcontratti": [matchAbiDateLte,limit6,unwindService,matchServiceName
                                          ,{$project:{abi:1,data:1,rel:{$slice: ["$servizio.rilevazioni",-1]}}}
                                          ,{$unwind:"$rel"}
                                          ,{$sort: {data:1}}
                                          ,{$group: {_id:"$abi",data:{$push: "$rel.firmati"},categories:{$push: "$data"}}}],
      "FEAcontabili": [matchAbiDateLte,limit6,unwindService,matchServiceName
                                          ,{$project:{abi:1,data:1,rel:{$slice: ["$servizio.rilevazioni",-1]}}}
                                          ,{$unwind:"$rel"}
                                          ,{$sort: {data:1}}
                                          ,{$group: {_id:"$abi",data:{$push: "$rel.firmati"},categories:{$push: "$data"}}}],
